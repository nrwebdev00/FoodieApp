...
// @desc Upload Recipe Picture To Cloudinary
// @route POST /api/upload/recipe
// @access PRIVATE - LOGIN
const uploadRecipePicture = asyncHandler(async(req, res, next) =>{
  const file = req.file

  cloudinary.v2.uploader.upload(file.path,
    { public_id: `recipeImage/${file.filename}`},)
    .then((result) =>{
      res.status(200).json({
        success: true,
        data: result
...
export { uploadProfilePicture, uploadRecipePicture }